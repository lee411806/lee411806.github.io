---
layout: post
title: "MCP 서버 설계 6편 - MCP 서버 구조 및 동작"
categories: [MCP서버 설계]
---

### MCP 서버란

LLM을 위한 API 서버. LLM이 외부 서비스를 사용할 수 있도록 도구(tool)를 제공한다.
...
LLM이 tool을 자동으로 아는 게 아니다. MCP 서버가 먼저 목록을 알려주는 구조다.
...
뭘 검색할지, 몇 번 tool을 호출할지는 LLM이 판단. MCP 서버는 창구 역할만 한다.
...
각 앱의 API 문서를 보고 뭘 제한할 수 있는지 먼저 확인 필요. API가 지원하는 범위 안에서만 제한 가능하다.
...
LLM이 직접 SSH 접속하는 게 아님. SSH 접속은 MCP 서버가 하고 LLM은 tool 호출만 한다. SSH 키/계정도 MCP 서버가 보유.

---

### 클라우드 서비스 연동 (구글 캘린더 예시)

로컬에 설치된 앱과 연동하는 게 아니라 **클라우드 서버에 직접 API로 접근**한다.
...
로컬 앱은 구글 캘린더 서버의 데이터를 보기 편하게 보여주는 클라이언트일 뿐. MCP 서버도 동일하게 구글 캘린더 서버에 직접 접근. OAuth 키만 있으면 된다.
